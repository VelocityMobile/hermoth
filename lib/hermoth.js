...
const getConfigFromArgs = (args) => {
  ...
  return {
    amqpEndpoint: args[0],
    amqpExchangeName: args[1],
    connectionRetryDelay: args[2],
  }
}
...
class Hermoth {
  constructor(...args) {
    ...
    this.connectionRetryDelay = Number(connectionRetryDelay || 2000)
    this.retryConnection = !(disableConnectionRetry || false)
    this.listeners = {}
  }
  ...
  async doConnect() {
    ...
    do {
      try {
        ...
        connected = false
        log.error(`Failed to connect to ${this.connDetails()}: ${err.code}. ${this.retryConnection ? `Retrying in ${this.connectionRetryDelay / 1000} seconds..` : ''}`) // eslint-disable-line max-len
