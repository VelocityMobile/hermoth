 ...
 */
export const doConnect = async (amqpEndpoint, connectRetryDelay) => {
    let conn;
    let connected = false;
    do {
        try {
            conn = await connect(amqpEndpoint); // eslint-disable-line no-await-in-loop
            connected = true
        } catch (err) {
            log.error(`Failed to connect to message broker (${err.code}). Retrying in ${connectRetryDelay / 1000} seconds...`); // eslint-disable-line max-len
            connected = false;
            await delay(connectRetryDelay); // eslint-disable-line no-await-in-loop
        }
    } while (!connected);
 ...
 */
export const consume = ({ content: blob }) => {
    let message;
    try {
 ...
 * @return {Promise}
 */
export const init = async (amqpEndpoint, amqpExchangeName, connectionRetryDelay = 2000) => {
    EXCHANGE_NAME = amqpExchangeName;
    const conn = await doConnect(amqpEndpoint, connectionRetryDelay);
    conn.once('close', () => {
        log.error('Connection to message broker lost.');
        return init(); // try to reconnect
    });
    ...
    await channel.assertExchange(amqpExchangeName, 'fanout', { durable: false });

    // Create queue if it doesn't exist. All messages are received from this queue.
    const queue = await channel.assertQueue('', { exclusive: true });
    channel.bindQueue(queue.queue, amqpExchangeName, '');

    return channel.consume(queue.queue, consume, { noAck: true });
};
