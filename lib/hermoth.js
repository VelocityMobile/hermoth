...
import { PrefixedLogger } from '~/lib/services/utils'

const log = new PrefixedLogger('pubsub')

const EXCHANGE_NAME = 'erb'
...
async function doConnect() {
    let conn
    let connected = false
    const CONNNECTION_RETRY_DELAY = 2000
    do {
        try {
            conn = await connect(process.env.AMQP_ENDPOINT_URL) // eslint-disable-line no-await-in-loop
            connected = true
        } catch (err) {
            log.error(`Failed to connect to message broker (${err.code}). Retrying in ${CONNNECTION_RETRY_DELAY / 1000} seconds...`) // eslint-disable-line max-len
            connected = false
            await delay(CONNNECTION_RETRY_DELAY) // eslint-disable-line no-await-in-loop
        }
    } while (!connected)
