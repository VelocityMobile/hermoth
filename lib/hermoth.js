...
class Hermoth {
  ...
  async consume({ content: blob }) {
    ...
    if (this.listeners[message.name]) {
      ...
      await Promise.all(this.listeners[message.name].map(listener => Promise.resolve(listener.call(this, message.payload, message.name, message.id)))) // eslint-disable-line max-len
